...
int interpret_command(int type, char* command)
{
    if(type == ADD_NODE)
    {
        struct node_to_add_ipv4 node = *(struct node_to_add_ipv4*)command;  // the only argument of ADD_NODE is the node to add (ip:port)
    ...
    {
        mark_for_removal(command);  // mark the node so that it can be removed later
        remove_nodes(); // if possible, remove all marked nodes

        return 0;
    }
    else if(type == LIST_NODES)
    {
        struct node_list_item_ipv4* node_list = get_node_list();    // get a list of nodes

        char* packet = node_list_packet(current_node_count, pool_size, 4, node_list);   // create a packet to be send
...
struct node_list_item_ipv4* get_node_list(void)
{
    ...
    {
        if(nodes[i].used != 0)  // we are only interested in used nodes
        {
